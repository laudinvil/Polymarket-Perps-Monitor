...
async function getJson(url) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      const res = await fetch(url, { headers: { "User-Agent": "btc-5m-oi-monitor/1.0", "Accept": "application/json" }, signal: AbortSignal.timeout(8000) });
...
async function getLargestBet(conditionId, start, end) {
  ...
  while (true) {
    ...
    for (const row of rows) {
      ...
      if (seconds < start) { reachedOlderTrades = true; continue; }
      if (seconds >= end) continue;
      if (side && side !== "BUY") continue;
      if (!largest || size > largest.size) {
        largest = { size: size, outcome: String(row.outcome || row.title || row.outcome_label || "").toUpperCase(), side: side || "BUY", price: Number(row.price), timestamp: seconds };
      }
    }
    console.log("Trades page=" + pages + " rows=" + rows.length + " totalRows=" + totalRows + " hasMore=" + Boolean(pagination.has_more) + " largest=" + (largest ? largest.size : "none"));
    if (reachedOlderTrades || !pagination.has_more || !pagination.next_cursor) break;
    cursor = pagination.next_cursor;
  }
  console.log("Largest BUY scan complete: pages=" + pages + " rows=" + totalRows + " result=" + (largest ? largest.size + " shares" : "none"));
...
async function monitorPeriod(start) {
  ...
  const nextState = {
    periodStart: start, snapshotOffset: TARGET_OFFSET, largestBetShares: largestBet.size,
    largestBetOutcome: outcome, largestBetSide: largestBet.side,
    largestBetPrice: Number.isFinite(largestBet.price) ? largestBet.price : null,
    largestBetTimestamp: largestBet.timestamp, lastOutcome: outcome, streak: streak,
    updatedAt: new Date().toISOString()
  };
  if (streak < 2) {
    console.log("Streak=" + streak + " outcome=" + outcome + "; no alert");
    writeState(nextState); gitCommitState(start); console.log("State saved for period=" + start);
    return;
  }
...
async function main() {
  ...
  while (true) {
    ...
    try { await monitorPeriod(current); }
    catch (err) {
