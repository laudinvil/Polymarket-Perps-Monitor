...
const STATE = path.resolve('.line-movement-state.json');
const HISTORY = path.resolve('line-movement-history.log');
...
function load(){ try{ Object.assign(state, JSON.parse(fs.readFileSync(STATE,'utf8'))); }catch{} state.events=state.events||{}; state.prices=state.prices||{}; state.alerted=state.alerted||{}; }
