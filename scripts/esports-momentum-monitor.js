...
const state = { alerted: {}, pm: {}, alertFingerprints: {} };
...
function recentForm(h, n) { return stats(h.slice(0, n)); }
function upsetSignal(a, b) {
  const candidates = [a, b].filter(t => t.s.recent5.sample === 5 && t.s.recent5.wins >= 3 && t.s.recent5.wins < 4 && t.s.recent3.losses >= 2);
  if (candidates.length !== 1) return null;
  const t = candidates[0];
  const o = t === a ? b : a;
  if (o.s.recent5.wins >= t.s.recent5.wins) return null;
  return { team: t, strategy: 'UPSET' };
}
function reversalSignal(t) {
  const current = t.s.recent3;
  const prior = t.s.prior5;
  if (current.sample !== 3 || prior.sample !== 5) return null;
  if (current.losses < 2 || current.losses > 3) return null;
  if (prior.wins < 3) return null;
  return { team: t, strategy: 'REVERSAL' };
}
...
async function inspect(m) { const ts=teams(m); if(ts.length!==2)return; const [ha,hb]=await Promise.all(ts.map(history)); if(ha.length<HISTORY||hb.length<HISTORY)return; const make=(team,h)=>{const s=stats(h); return {...team,s:{...s,recent3:recentForm(h,3),recent5:recentForm(h,5),prior5:stats(h.slice(3,8))}};}; const a=make(ts[0],ha),b=make(ts[1],hb); const upset=upsetSignal(a,b); const reversalA=reversalSignal(a),reversalB=reversalSignal(b); const signals=[upset,reversalA,reversalB].filter(Boolean); const s=signals[0]||null; console.log(JSON.stringify({matchId:m.id,game:m.videogame?.slug,beginAt:m.begin_at,teams:[a.name,b.name],a:a.s,b:b.s,upset:upset?.team.name||null,reversal:[reversalA?.team.name,reversalB?.team.name].filter(Boolean),signal:s?.strategy||null})); log({type:'match',matchId:m.id,game:m.videogame?.slug,beginAt:m.begin_at,teams:[a.name,b.name],a:a.s,b:b.s,signal:s?.strategy||null,signalTeam:s?.team.name||null}); if(!s||state.alerted[m.id])return; const event=await polymarket(a,b,m.begin_at); const url=event?.slug?`https://polymarket.com/event/${event.slug}`:null; if(!url){console.log(`NO POLYMARKET ${a.name} vs ${b.name}`);return;} s.url=url; const fingerprint=alertFingerprint(m,a,b,s,url); if(state.alertFingerprints[fingerprint]){ console.log(`DUPLICATE SUPPRESSED ${s.strategy} ${a.name} vs ${b.name} ${s.team.name} ${url}`); log({type:'duplicate_suppressed',matchId:m.id,team:s.team.name,strategy:s.strategy,url,fingerprint}); state.alerted[m.id]={at:Date.now(),team:s.team.name,strategy:s.strategy,url,suppressed:true}; save(); return; } await sendTelegramMessage(alertText(m,a,b,s)); state.alerted[m.id]={at:Date.now(),team:s.team.name,strategy:s.strategy,url}; state.alertFingerprints[fingerprint]={at:Date.now(),matchId:m.id,team:s.team.name,strategy:s.strategy,url}; save(); log({type:'alert',matchId:m.id,team:s.team.name,strategy:s.strategy,url,fingerprint}); console.log(`ALERT ${s.strategy} ${a.name} vs ${b.name} ${s.team.name} ${url}`); }
...
async function main(){console.log(`ESPORTS UPSET+REVERSAL MONITOR STARTED poll=${POLL/1000}s lookahead=${LOOKAHEAD}h`);while(true){const t=Date.now();try{await poll();}catch(e){console.error(`LOOP FAILED ${e.stack||e.message}`);}await sleep(Math.max(1000,POLL-(Date.now()-t)));}}
