...
async function inspect(m) { const ts=teams(m); if(ts.length!==2)return; const [ha,hb]=await Promise.all(ts.map(history)); if(ha.length<HISTORY||hb.length<HISTORY)return; const make=(team,h)=>{const s=stats(h); return {...team,s:{...s,recent3:recentForm(h,3),recent5:recentForm(h,5),prior5:stats(h.slice(3,8))}};}; const a=make(ts[0],ha),b=make(ts[1],hb); const upset=upsetSignal(a,b); const reversalA=reversalSignal(a),reversalB=reversalSignal(b); const signals=[upset,reversalA,reversalB].filter(Boolean); const s=signals[0]||null; console.log(JSON.stringify({matchId:m.id,game:m.videogame?.slug,beginAt:m.begin_at,teams:[a.name,b.name],a:a.s,b:b.s,upset:upset?.team.name||null,reversal:[reversalA?.team.name,reversalB?.team.name].filter(Boolean),signal:s?.strategy||null})); log({type:'match',matchId:m.id,game:m.videogame?.slug,beginAt:m.begin_at,teams:[a.name,b.name],a:a.s,b:b.s,signal:s?.strategy||null,signalTeam:s?.team.name||null}); if(!s||state.alerted[m.id])return; const event=await polymarket(a,b,m.begin_at); const url=event?.slug?`https://polymarket.com/event/${event.slug}`:null; if(!url){console.log(`NO POLYMARKET ${a.name} vs ${b.name}`);return;} s.url=url; await sendTelegramMessage(alertText(m,a,b,s)); state.alerted[m.id]={at:Date.now(),team:s.team.name,strategy:s.strategy,url}; save(); log({type:'alert',matchId:m.id,team:s.team.name,strategy:s.strategy,url}); console.log(`ALERT ${s.strategy} ${a.name} vs ${b.name} ${s.team.name} ${url}`); }
