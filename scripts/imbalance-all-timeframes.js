...
function persistStatus() {
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      seenLiquidations: [...state.seenLiquidations].slice(-5000),
...
function restoreState() {
  try {
    ...
    state.longCount = Number(saved.longCount || 0);
    state.shortCount = Number(saved.shortCount || 0);
    state.periodAlreadyAlerted = Boolean(saved.periodAlreadyAlerted);
    state.seenLiquidations = new Set(Array.isArray(saved.seenLiquidations) ? saved.seenLiquidations : []);
...
function collectCurrentPeriodEvents(feeds, now) {
  ...
  for (const item of events) {
    ...
    if (item.side === 'LONG') state.longCount += 1;
    ...
    if (!state.periodAlreadyAlerted && (crossedLong || crossedShort)) {
      ...
      const beforeLeader = crossedLong ? 'SHORT' : 'LONG';
      state.periodAlreadyAlerted = true;
      persistStatus();
      console.log(`5m IMBALANCE CROSS symbol=${item.symbol} direction=${direction} before=${beforeLong}/${beforeShort} after=${state.longCount}/${state.shortCount} displaced=${beforeLeader} GLOBAL_PERIOD_LOCK=CLOSED`);
...
function enqueueAlert(symbol, direction, currentPeriod, beforeLong, beforeShort, longCount, shortCount) {
  alertSendChain = alertSendChain.then(async () => {
    ...
    try {
      ...
      const message = [
        `🔥 ${symbol} · 5M`,
        `LIQUIDATION IMBALANCE FLIP · ${direction}`,
        `Before: LONG ${beforeLong} · SHORT ${beforeShort}`,
        `After: LONG ${longCount} · SHORT ${shortCount}`,
        `Trigger: ${symbol} liquidation`,
        `Period: ${new Date(currentPeriod).toLocaleString('en-GB', { timeZone: 'Europe/Kyiv', hour12: false })}`,
      ...
      persistStatus();
      appendHistory({
        type: 'global_liquidation_imbalance_cross',
        timeframe: '5m',
        symbol,
        direction,
        currentPeriod,
      ...
      console.log(`5m GLOBAL IMBALANCE ALERT SENT symbol=${symbol} direction=${direction} before=${beforeLong}/${beforeShort} after=${longCount}/${shortCount} market=${market?.url || 'NONE'}`);
    } catch (error) {
      console.warn(`5m GLOBAL IMBALANCE ALERT FAILED ${symbol}: ${error.message}`);
    }
...
function initializePeriod(current) {
  if (state.periodStart === null) {
    ...
    persistStatus();
    console.log(`5m GLOBAL IMBALANCE START ${new Date(current).toISOString()} symbols=${SYMBOLS.join(',')}`);
    return;
  }

  if (state.periodStart === current) return;

  console.log(`5m PERIOD CLOSE period=${new Date(state.periodStart).toISOString()} long=${state.longCount} short=${state.shortCount}`);
  state.periodStart = current;
  state.longCount = 0;
  state.shortCount = 0;
  state.periodAlreadyAlerted = false;
...
async function processTimeframe(feeds, now) {
  ...
  if (!state.initialized) {
    ...
    persistStatus();
    console.log(`INITIAL 5m GLOBAL IMBALANCE BASELINE READY current=${new Date(current).toISOString()} long=${state.longCount} short=${state.shortCount}`);
  } else if (added) {
    persistStatus();
  }
}
...
function main() {
  restoreState();
  console.log(`5m GLOBAL LIQUIDATION IMBALANCE MONITOR STARTED; symbols=${SYMBOLS.join(',')}; LONG and SHORT are accumulated across all 7 coins; alert only when the previously smaller side becomes strictly larger because of a liquidation event; one alert per 5m period.`);

  (async () => {
    while (true) {
      const now = Date.now();
      try {
        await processTimeframe(await fetchAllFeeds(), now);
      } catch (error) {
