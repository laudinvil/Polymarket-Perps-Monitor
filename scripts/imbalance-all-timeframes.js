...
async function loadState() {
  ...
  try { const response = await githubRequest(); if (!response?.content) return; const state = JSON.parse(Buffer.from(response.content.replace(/\s/g, ''), 'base64').toString('utf8')); loadPersistedState(state); console.log('STATE LOADED; sent alerts restored, ALL streaks reset to 0; thresholds=5m:3+,15m:2+,1h:2+,4h:2+'); }
...
async function saveGlobalState() {
  ...
  for (let attempt = 1; attempt <= 5; attempt += 1) try {
    ...
    await githubRequest('PUT', { message: 'Set 5m streak alert threshold to 3+', content: Buffer.from(JSON.stringify(state, null, 2)).toString('base64'), branch: 'monitor-status', ...(response?.sha ? { sha: response.sha } : {}) }); return;
...
async function main() {
  await loadState(); console.log('LIQUIDATION STREAK MONITOR STARTED; alerts only for largest streak in each completed period; ties all alert; thresholds=5m:3+,15m:2+,1h:2+,4h:2+; ALL streaks start from 0 after restart; streaks require consecutive completed buckets; zero LONG and zero SHORT resets; sends serialized with 5s minimum gap; 5m/15m/1h/4h');
