...
async function backfillPeriod(v) {
  const trades = await fetchFullPeriodTrades(v.market, v.start); if (trades === null) return false;
  let up = 0, down = 0, lu = null, ld = null, luTs = -Infinity, ldTs = -Infinity;
  const upToken = String(v.market.tokenIds.UP), downToken = String(v.market.tokenIds.DOWN);
  for (const trade of trades) {
    const asset = String(trade.asset || ''), p = Number(trade.price), q = Number(trade.size), ts = Number(trade.timestamp);
    if (!Number.isFinite(p) || !Number.isFinite(q) || q <= 0) continue;
    if (asset === upToken) { up += p * q; if (ts >= luTs) { lu = p; luTs = ts; } }
    else if (asset === downToken) { down += p * q; if (ts >= ldTs) { ld = p; ldTs = ts; } }
  }
  v.trades = trades.length; v.up = up; v.down = down; v.lu = lu; v.ld = ld; v.backfilled = true; return true;
}
async function checkBoundary(currentStart) {
  const justFinishedStart = currentStart - PERIOD, previousStart = currentStart - PERIOD * 2;
  const current = completed.get(key('BTC', justFinishedStart)), previous = completed.get(key('BTC', previousStart));
  if (!current || current.alertChecked) return;
  try {
    const ok = await backfillPeriod(current); if (!ok) return;
...
async function refresh() {
  const t = start(), symbol = 'BTC', market = await findMarketByEpoch(symbol, t, '5m');
  if (!market?.tokenIds?.UP || !market?.tokenIds?.DOWN) { console.log(`[crowd-flow] BTC market unavailable period=${t}`); return; }
  ...
  tokens.set(market.tokenIds.UP, { k, o:'UP' }); tokens.set(market.tokenIds.DOWN, { k, o:'DOWN' });
  const previousStart = t - PERIOD, previous = markets.get(key(symbol, previousStart));
  if (previous && !completed.has(key(symbol, previousStart))) completed.set(key(symbol, previousStart), { ...previous });
  const olderStart = t - PERIOD * 2, older = markets.get(key(symbol, olderStart));
  ...
  if (socket?.readyState === WebSocket.OPEN) {
    ...
    if (ids.length) { socket.send(JSON.stringify({ assets_ids:ids, type:'market' })); console.log(`[crowd-flow] subscribed tokens=${ids.length} symbol=BTC period=${t}`); }
  }
}
function event(x) {
  if (x?.event_type !== 'last_trade_price') return; const m = tokens.get(String(x.asset_id)); if (!m) return;
  const v = markets.get(m.k); if (!v || v.start !== start()) return; const id = `${x.asset_id}:${x.timestamp}:${x.transaction_hash || x.price}`;
  if (seen.has(id)) return; seen.add(id); const p = Number(x.price), q = Number(x.size), n = p * q; if (!Number.isFinite(n) || n <= 0) return;
  v.trades += 1; if (m.o === 'UP') { v.up += n; v.lu = p; } else { v.down += n; v.ld = p; }
}
function diagnostics() { const t = start(), v = markets.get(key('BTC', t)); if (v) console.log(`[crowd-flow] DIAG BTC=${v.trades}`); }
function connect() {
  socket = new WebSocket(WS); socket.onopen = () => refresh();
  socket.onmessage = e => { try { const x=JSON.parse(e.data); (Array.isArray(x)?x:[x]).forEach(event); } catch {} };
  socket.onclose = () => setTimeout(connect, 3000); socket.onerror = () => { try { socket.close(); } catch {} };
}
(async () => { console.log('[crowd-flow] start BTC'); await refresh(); connect(); setInterval(() => refresh().catch(e => console.error('[crowd-flow]', e.message)), 30000); setInterval(diagnostics, 60000); })();
