...
async function sofascoreLive(){
  try{
    const x=await json(SOFASCORE_LIVE_URL,7000);
    if(Array.isArray(x?.events))return x;
    throw new Error("Sofascore live response has no events array");
  }catch(err){
    log(JSON.stringify({event:"sofascore_live_failed",message:err.message}));
    throw err;
  }
}
...
function exactScore11Yes(e){
  ...
  for(const m of markets){
    ...
    for(let i=0;i<os.length;i++){
      const o=norm(os[i]),rawO=t(os[i]).toLowerCase();
      if((is11||exactSection)&&o==="yes"&&ps[i]>=0&&ps[i]<=1&&is11)return ps[i];
...
async function claim(key){
  const r=await fetch(CONVEX+"/football/claim",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({monitor:"polymarket-football",marketSlug:key}),signal:AbortSignal.timeout(2000)});
  return r.status===200?await r.json():{claimed:false};
}
async function release(key){
  try{await fetch(CONVEX+"/football/release",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({monitor:"polymarket-football",marketSlug:key}),signal:AbortSignal.timeout(2000)});}catch{}
}
async function telegram(message,replyTo=null){
  ...
  const r=await fetch("https://api.telegram.org/bot"+token+"/sendMessage",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body),signal:AbortSignal.timeout(10000)});
...
async function saveId(key,id){
  try{await fetch(CONVEX+"/football/telegram-message",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({monitor:"polymarket-football",marketSlug:key,messageId:id}),signal:AbortSignal.timeout(2000)});}catch{}
}
async function discoverLiveZeroZero(){
  const live=await sofascoreLive(),events=arr(live?.events),zeroZero=events.filter(scoreZeroZero),inprogress=events.filter(e=>e?.status?.type==="inprogress"),eligible=events.filter(e=>e?.status?.type==="inprogress"&&scoreZeroZero(e)&&sofascoreMinute(e)>=1);
  ...
  for(const s of events){
    ...
    try{
      ...
      if(!pm){log(JSON.stringify({event:"sofascore_00_no_polymarket",teams:[home,away],minute:sofascoreMinute(s),candidateCount:candidates.length}));continue;}
...
async function sendNext(row,score){
  ...
  try{
    ...
    return true;
  }catch(err){await release(alertKey);log(JSON.stringify({event:"telegram_alert_failed",type:"NEXT",key:row.key,message:err.message}));return false;}
}
async function sendLoss(row,score){
  const alertKey=row.key+":LOSS",c=await claim(alertKey); if(!c.claimed)return false;
  const message=["⚽ LOSS","","",row.home+" vs "+row.away,"SCORE: 0–0","","➡️ OPEN MATCH","https://polymarket.com"+row.href].join("\n");
  try{const sent=await telegram(message,c.replyToMessageId??null);await saveId(alertKey,sent.message_id);log(JSON.stringify({event:"telegram_alert_sent",type:"LOSS",key:row.key,score,messageId:sent.message_id}));return true;}
  catch(err){await release(alertKey);log(JSON.stringify({event:"telegram_alert_failed",type:"LOSS",key:row.key,message:err.message}));return false;}
}
async function sendLive(row,score){
  const alertKey=row.key+":LIVE",c=await claim(alertKey); if(!c.claimed)return false;
  const message=["⚽ LIVE","",row.home+" vs "+row.away,"",row.odds,"","➡️ OPEN MATCH","https://polymarket.com"+row.href].join("\n");
  try{const sent=await telegram(message,c.replyToMessageId??null);await saveId(alertKey,sent.message_id);log(JSON.stringify({event:"telegram_alert_sent",type:"LIVE",key:row.key,score,messageId:sent.message_id}));return true;}
  catch(err){await release(alertKey);log(JSON.stringify({event:"telegram_alert_failed",type:"LIVE",key:row.key,message:err.message}));return false;}
}
async function sendSell(row,score){
  const alertKey=row.key+":SELL::"+score.home+"-"+score.away,c=await claim(alertKey); if(!c.claimed)return false;
  ...
  try{const sent=await telegram(message,c.replyToMessageId??null);await saveId(alertKey,sent.message_id);log(JSON.stringify({event:"telegram_alert_sent",type:"SELL",key:row.key,score,messageId:sent.message_id}));return true;}
  catch(err){await release(alertKey);log(JSON.stringify({event:"telegram_alert_failed",type:"SELL",key:row.key,message:err.message}));return false;}
}
async function scan(){
