...
function teamsFromEvent(e){const title=t(e?.title||e?.question);if(Array.isArray(e?.homeTeam)&&e.homeTeam.length===2)return e.homeTeam;const h=t(e?.homeTeam||e?.home_team),a=t(e?.awayTeam||e?.away_team);if(h&&a)return [h,a];const m=title.match(/^(.+?)\s+(?:vs\.?|v\.?|versus)\s+(.+?)(?:\s+-\s+.*)?$/i);return m?[m[1],m[2]]:["",""];}
function eventIsFixture(e){const [h,a]=teamsFromEvent(e);return !!h&&!!a&&/\s(?:vs\.?|v\.?|versus)\s/i.test(t(e?.title||e?.question));}
function cardAround(page,home,away){const p=norm(page),h=norm(home),a=norm(away);if(!p||!h||!a)return null;const aliases=(x)=>[x,x.replace(/^cd\s+/,"")].filter(Boolean);let best=null;for(const hh of aliases(h)){const hi=p.indexOf(hh);if(hi<0)continue;for(const aa of aliases(a)){const ai=p.indexOf(aa,hi+hh.length);if(ai<0||ai-hi>1000)continue;const start=Math.max(0,hi-350),end=Math.min(p.length,ai+aa.length+450),card=p.slice(start,end);if(/\b(?:1h|2h|ht|et|aet|live|playing|in progress|penalties|pen)\b/.test(card)){best=card;break;}}if(best)break;}return best;}
function scoreFromCard(card){if(!card)return null;let m=card.match(/\b(\d{1,2})\s*[-–]\s*(\d{1,2})\b/);if(!m)m=card.match(/\b(\d{1,2})\s+(?:vs|v)\s+(\d{1,2})\b/);if(!m)return {home:0,away:0};const h=Number(m[1]),a=Number(m[2]);return h>=0&&a>=0&&h<=20&&a<=20?{home:h,away:a}:{home:0,away:0};}
...
async function scan(){
  ...
  const page=cleanHtml(html);
  const hrefs=links(html).filter(x=>isSoccerHref(x.href)).slice(0,50);
  console.log(JSON.stringify({event:"live_page_snapshot",hrefCount:hrefs.length,source:LIVE_URL}));
  for(const item of hrefs){
    ...
    const [home,away]=teamsFromEvent(e);const card=cardAround(page,home,away);if(!card)continue;
    const score=scoreFromCard(card);const key=t(e.id||e.eventId||e.slug);if(!key)continue;
    const url="https://polymarket.com"+item.href;
    const odds=oneXTwo(e,home,away);
    const previous=lastScores.get(key);lastScores.set(key,`${score.home}-${score.away}`);
    console.log(JSON.stringify({event:"live_match",key,teams:[home,away],score,odds}));
    const isFirst=!previous;
    const changed=previous && previous!==`${score.home}-${score.away}`;
    if(!isFirst&&!changed)continue;
    const type=isFirst?"LIVE":"SELL";const alertKey=`${key}:${type}${isFirst?"":"::"+score.home+"-"+score.away}`;const c=await claim(alertKey);if(!c.claimed)continue;
