...
function loadState() { try { return JSON.parse(fs.readFileSync(STATE_FILE, 'utf8')); } catch { return { strategy: 'StreakHit', timeframes: {}, alerts: {} }; } }
...
function winnerFromMarket(market) {
  ...
  const up = outcomes.indexOf('UP'); const down = outcomes.indexOf('DOWN');
...
async function polyBacktestMarkets(timeframe, limit) {
  ...
  const response = await fetch(`${POLYBACKTEST_BASE}?type=${encodeURIComponent(timeframe)}&limit=${limit}`, { headers: { 'X-API-Key': apiKey, Accept: 'application/json' } });
  if (!response.ok) { const text = await response.text(); throw new Error(`PolyBackTest ${response.status}: ${text.slice(0, 500)}`); }
...
function runBacktest(rows, timeframe) {
  ...
  let direction = null; let streak = 0; const hits = [];
  for (const row of ordered) {
    if (row.winner === direction) streak += 1; else { direction = row.winner; streak = 1; }
    if (streak >= cfg.minStreak) hits.push({ marketId: row.market_id || row.marketId, slug: row.slug, timeframe, startTime: row.start_time || row.startTime, endTime: row.end_time || row.endTime, direction, streak, threshold: cfg.minStreak, newStreak: streak > cfg.minStreak });
  }
...
async function live() {
  ...
  while (true) {
    for (const timeframe of Object.keys(TIMEFRAMES)) { try { await processTimeframe(state, timeframe, Date.now()); } catch (error) { console.warn(`StreakHit ${timeframe} failed: ${error.message}`); } }
