...
async function getPaperEntry(nextMarket, outcome) {
  ...
  console.log(`[5M] PAPER entry NEXT+4 market=${nextMarket.slug} outcome=${outcome} clob_mid=${midpoint ?? 'N/A'} gamma=${Number.isFinite(gammaPrice) ? gammaPrice : 'N/A'} selected=${price}`);
...
async function alertForCombo(firstEvent, secondEvent, state, alertPeriod) {
  ...
  const nextUrl = nextMarket?.url || `https://polymarket.com/event/${symbol.toLowerCase()}-updown-5m-${Math.floor((alertPeriod + 4 * PERIOD_MS) / 1000)}`;
  ...
  const message = [
    ...
    ...(paperTrade ? [`📈 PAPER TRADE · $${PAPER_USD.toFixed(2)}`, `BUY ${paperTrade.outcome} @ ${paperTrade.entryPrice.toFixed(4)}`, `Shares: ${paperTrade.shares.toFixed(4)}`] : ['📈 PAPER TRADE · next+4 market entry unavailable']),
    `➡️ NEXT+4 · Polymarket 5M`, nextUrl,
  ].join('\n');
  try {
  ...
  console.log(`[5M] COMBO ALERT ${symbol} ${firstSide}->${secondSide} firstPeriod=${formatTime(periodStart(firstTs))} secondPeriod=${formatTime(periodStart(secondTs))} paper=${paperTrade?.outcome || 'N/A'} entry=${paperTrade?.entryPrice ?? 'N/A'} next+4=${nextUrl}`);
  return true;
}
