...
async function scan() {
  ...
  if (Date.now() - previous < ALERT_COOLDOWN_MS) {
    console.log(`[IMMINENT][COOLDOWN] ${item.coin} ${item.side} ${item.distancePct.toFixed(2)}%`);
    return alerts;
  }
...
function nextPolymarket5mUrl(coin) {
  const now = Date.now();
...
async function main() {
  ...
  console.log('RULE: one alert only — closest liquidation across all 7 coins/users');
  console.log('MARKET: next Polymarket 5M');
  await scan();
  if (RUN_ONCE) return;
  while (!stopping) {
    const now = Date.now();
    const nextBoundary = Math.floor(now / 300000) * 300000 + 300000;
    const delay = Math.max(1000, nextBoundary - now);
    console.log(`[IMMINENT][SCHEDULER] next 5M boundary=${new Date(nextBoundary).toISOString()} delay=${delay}ms`);
