...
async function getMarketContexts() {
  const response = await fetch(HL_INFO_URL, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ type: 'metaAndAssetCtxs', dex: '' }) });
  if (!response.ok) throw new Error(`Hyperliquid metaAndAssetCtxs HTTP ${response.status}`);
  const body = await response.json();
  const meta = body?.[0]; const contexts = body?.[1]; const map = new Map();
  if (!meta?.universe || !Array.isArray(contexts)) return map;
  for (let i = 0; i < meta.universe.length; i++) { const coin = meta.universe[i]?.name; const mark = Number(contexts[i]?.markPx); if (coin && Number.isFinite(mark) && mark > 0) map.set(coin, mark); }
  return map;
}

async function getUserState(user) {
  const response = await fetch(HL_INFO_URL, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ type: 'clearinghouseState', user, dex: '' }) });
...
function parsePosition(position, markPx) {
  const size = Number(position?.szi); const liqPx = Number(position?.liquidationPx); const positionValue = Math.abs(Number(position?.positionValue));
...
async function scan() {
  console.log(`[IMMINENT][SCAN] threshold=${THRESHOLD_PCT}% interval=${SCAN_INTERVAL_MS}ms lookback=${Math.round(CANDIDATE_LOOKBACK_MS / 60000)}m`);
  const [candidates, marks] = await Promise.all([discoverCandidates(), getMarketContexts()]); const alerts = [];
  for (const coin of ASSETS) {
    const users = candidates.get(coin) || [];
    const states = await Promise.all(users.map(async user => { try { return { user, state: await getUserState(user) }; } catch (error) { console.error(`[IMMINENT][USER_ERROR] ${coin} ${user}: ${error?.message ?? error}`); return null; } }));
    for (const item of states) { if (!item) continue; const positions = Array.isArray(item.state?.assetPositions) ? item.state.assetPositions : []; for (const wrapper of positions) { const position = wrapper?.position; if (!position || position.coin !== coin) continue; const parsed = parsePosition(position, marks.get(coin)); if (parsed) alerts.push({ user: item.user, ...parsed }); } }
  }
  alerts.sort((a, b) => a.distancePct - b.distancePct || b.positionValue - a.positionValue); console.log(`[IMMINENT][FOUND] qualifying=${alerts.length}`); if (alerts.length) console.log(`[IMMINENT][TOP] ${JSON.stringify(alerts[0])}`);
  const item = alerts[0]; if (!item) return alerts; const key = `${item.user}:${item.coin}:${item.side}`; const previous = alerted.get(key) || 0;
  if (Date.now() - previous < ALERT_COOLDOWN_MS) { console.log(`[IMMINENT][COOLDOWN] ${item.coin} ${item.side} ${item.distancePct.toFixed(2)}%`); return alerts; }
  await sendAlert(item); alerted.clear(); alerted.set(key, Date.now()); return alerts;
}
...
async function sendAlert(item) {
  ...
  const text = `⚠️ ${emoji} #${item.coin} Imminent ${item.side} Liquidation: ${fmtUsd(item.positionValue)} @ ${item.distancePct.toFixed(2)}% away (liq: ${fmtUsd(item.liqPx)})\n\n▶️ NEXT POLYMARKET 5M\n${nextPolymarket5mUrl(item.coin)}`;
  ...
  const raw = await response.text(); console.log(`[IMMINENT][TELEGRAM] HTTP ${response.status}: ${raw.slice(0, 300)}`); if (!response.ok) throw new Error(`Telegram HTTP ${response.status}: ${raw}`);
}

async function main() {
  assertConfig(); console.log('=== IMMINENT LIQUIDATION MONITOR ==='); console.log(`THRESHOLD: ${THRESHOLD_PCT}%`); console.log(`SCAN INTERVAL: ${SCAN_INTERVAL_MS}ms`); console.log('RULE: one alert only — closest liquidation across all 7 coins/users'); console.log('MARKET: next Polymarket 5M'); await scan();
  if (RUN_ONCE) return;
