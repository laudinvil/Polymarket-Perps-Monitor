...
function loadState() {
  try {
    ...
    for (const key of Object.keys(parsed?.markets || {})) alertedMarkets.add(key);
    console.log(`[IMMINENT][STATE] loaded positions=${alertedPositions.size} markets=${alertedMarkets.size} from ${STATE_FILE}`);
  } catch (error) {
...
function saveState() {
  ...
  const markets = {};
  for (const key of alertedMarkets) markets[key] = { alertedAt: new Date().toISOString() };
  fs.mkdirSync('src', { recursive: true });
  fs.writeFileSync(STATE_FILE, JSON.stringify({ version: 3, rule: 'one position = one alert permanently; one market = one alert permanently', positions, markets }, null, 2) + '\n');
  console.log(`[IMMINENT][STATE] saved positions=${alertedPositions.size} markets=${alertedMarkets.size}`);
}
...
async function scan() {
  console.log(`[IMMINENT][SCAN] threshold=${THRESHOLD_PCT}% interval=${SCAN_INTERVAL_MS}ms`);
  ...
  for (const coin of ASSETS) {
    ...
    for (const item of states) {
      ...
      for (const wrapper of positions) {
        ...
        if (parsed) {
          const market = nextPolymarket5mUrl(coin);
          alerts.push({ user: item.user, key: positionKey(item.user, position), market, ...parsed });
        }
      }
    }
  }
  alerts.sort((a, b) => a.distancePct - b.distancePct || b.positionValue - a.positionValue);
  const unseen = alerts.filter(x => !alertedPositions.has(x.key) && !alertedMarkets.has(x.market));
  console.log(`[IMMINENT][FOUND] qualifying=${alerts.length} unseen=${unseen.length} permanentlyAlertedPositions=${alertedPositions.size} permanentlyAlertedMarkets=${alertedMarkets.size}`);
  if (!unseen.length) return alerts;
  const item = unseen[0];
  await sendAlert(item);
  alertedPositions.add(item.key);
  alertedMarkets.add(item.market);
  saveState();
  console.log(`[IMMINENT][PERMANENT_FILTER] recorded position=${item.key} market=${item.market}`);
  return alerts;
}
function fmtUsd(value) { return `$${Number(value).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`; }
async function sendAlert(item) { const emoji = item.side === 'Long' ? '🔴' : '🟢'; const text = `${emoji} #${item.coin} Imminent ${item.side} Liquidation: ${fmtUsd(item.positionValue)} @ ${item.distancePct.toFixed(2)}% away (liq: ${fmtUsd(item.liqPx)})\n\n▶️ NEXT POLYMARKET 5M\n${item.market}`; console.log(`[IMMINENT][ALERT] ${text} user=${item.user} position=${item.key} market=${item.market}`); const response = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ chat_id: TELEGRAM_CHAT_ID, text, disable_web_page_preview: true }) }); const raw = await response.text(); console.log(`[IMMINENT][TELEGRAM] HTTP ${response.status}: ${raw.slice(0, 300)}`); if (!response.ok) throw new Error(`Telegram HTTP ${response.status}: ${raw}`); }
async function main() { assertConfig(); loadState(); console.log('=== IMMINENT LIQUIDATION MONITOR ==='); console.log(`THRESHOLD: ${THRESHOLD_PCT}%`); console.log('RULE: ONE POSITION = ONE ALERT, PERMANENTLY; ONE POLYMARKET = ONE ALERT, PERMANENTLY'); console.log('POSITION KEY: wallet + coin + side + entry price'); console.log('MARKET KEY: exact NEXT POLYMARKET 5M URL'); console.log('MARKET: next Polymarket 5M'); await scan(); if (RUN_ONCE) return; while (!stopping) { const now = Date.now(); const nextBoundary = Math.floor(now / 300000) * 300000 + 300000; const delay = Math.max(1000, nextBoundary - now); console.log(`[IMMINENT][SCHEDULER] next 5M boundary=${new Date(nextBoundary).toISOString()} delay=${delay}ms`); await new Promise(resolve => setTimeout(resolve, delay)); if (stopping) break; try { await scan(); } catch (error) { console.error('[IMMINENT][SCAN_ERROR]', error?.stack ?? error); } } }
