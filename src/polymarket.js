...
function parseMarketPrices(market) {
  ...
  try { if (typeof outcomePrices === 'string') outcomePrices = JSON.parse(outcomePrices); } catch { outcomePrices = null; }
  if (!Array.isArray(outcomes) || !Array.isArray(outcomePrices)) return {};
  const prices = {};
  outcomes.forEach((outcome, index) => {
    const value = Number(outcomePrices[index]);
    if (Number.isFinite(value)) prices[String(outcome).toUpperCase()] = value;
  });
  return prices;
}
async function findMarketBySlug(slug) {
  ...
  return {
    ...
    prices: parseMarketPrices(market),
  };
}
...
async function findNextMarket(symbol, now = Date.now(), timeframe = '5m') {
  ...
  if (fallbackUrl) return { slug: fallbackUrl.slice(`${MARKET_BASE_URL}/`.length), url: fallbackUrl, synthetic: true, prices: {} };
  return null;
}
