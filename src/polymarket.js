...
async function clobMidpoint(tokenId) {
  ...
  const data = await getJson(`${CLOB_BASE_URL}/midpoint?token_id=${encodeURIComponent(tokenId)}`);
  const value = Number(data?.mid);
  return Number.isFinite(value) ? value : null;
}
async function findClobMidpoint(market, outcome) {
  const tokenId = market?.tokenIds?.[String(outcome || '').toUpperCase()];
  return clobMidpoint(tokenId);
}
