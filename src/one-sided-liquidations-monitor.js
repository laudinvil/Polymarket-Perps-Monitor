...
const STATE_PATH = '.one-sided-liquidations-state.json';
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = require('https').request(GITHUB_API, {
      ...
      headers: {
        ...
        'User-Agent': 'polymarket-one-sided-liquidations-monitor',
...
async function saveState(sent, sha) {
  ...
  const body = { message: 'Persist one-sided liquidation alerts state', content, branch: process.env.GITHUB_REF_NAME || 'main' };
  if (sha) body.sha = sha;
...
function countOneSided(rows) {
  return rows.filter(row => (row.long > 0 && row.short === 0) || (row.short > 0 && row.long === 0));
}
...
async function alertFor(matches, timeframe, currentBucket, closedBucket) {
  for (const item of matches) {
    ...
    console.log(JSON.stringify({ type: 'one_sided_liquidations_alert', symbol: item.symbol, timeframe, closedBucket: new Date(closedBucket).toISOString(), long: item.long, short: item.short, market: market?.url || null }));
  }
}
...
async function main() {
  ...
  console.log(JSON.stringify({ type: 'one_sided_diagnostics_5m', bucket: new Date(closed5).toISOString(), symbols: rows5 }));
  const key5 = `5m:${closed5}`;
  if (!sent[key5]) {
    const matches5 = countOneSided(rows5);
    console.log(JSON.stringify({ type: 'one_sided_liquidations_5m', bucket: new Date(closed5).toISOString(), matches: matches5 }));
  ...
  console.log(JSON.stringify({ type: 'one_sided_diagnostics_15m', bucket: new Date(closed15).toISOString(), symbols: rows15 }));
  const key15 = `15m:${closed15}`;
  if (!sent[key15]) {
    const matches15 = countOneSided(rows15);
    console.log(JSON.stringify({ type: 'one_sided_liquidations_15m', bucket: new Date(closed15).toISOString(), matches: matches15 }));
...
main().catch(error => {
  console.error(`ONE-SIDED LIQUIDATIONS MONITOR FAILED: ${error.stack || error.message}`);
  process.exitCode = 1;
});
