  ...
  if (missingSymbols.length > 0) {
    ...
    if (fresh - fallbackCache.fetchedAt >= FALLBACK_REFRESH_MS) {
      const results = await Promise.all(missingSymbols.map(async symbol => {
        ...
        catch (error) { console.warn(`MarginPad live fallback ${symbol}: ${error.message}`); return [symbol, []]; }
      }));
...
function aggregateEvents(events, symbols = DEFAULT_SYMBOLS, now = Date.now()) {
  ...
  for (const event of events || []) {
    ...
    if (!rows.has(key)) rows.set(key, { bucket, symbol, events: 0, longEvents: 0, shortEvents: 0 });
  ...
  return [...rows.values()].sort((a, b) => b.events - a.events);
}

function selectWinner(rows, bucket) {
  return rows.filter(row => row.bucket === bucket).sort((a, b) => b.events - a.events)[0] || null;
}
