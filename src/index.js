...
async function finalizePeriod(period, start) {
  ...
  for (const asset of ASSETS) {
    ...
    if (cvd !== 0) candidates.push({ asset, direction: cvd > 0 ? 'POSITIVE' : 'NEGATIVE', value: Math.abs(cvd), cvd });
  }
...
function addCvd(asset, isBuyerAggressor, usd, now) { const state = getState(asset); rollBuckets(asset, state, now); for (const period of Object.keys(WINDOWS)) { const bucket = state[period]; if (isBuyerAggressor) bucket.buyVolume += usd; else bucket.sellVolume += usd; bucket.cvd = bucket.buyVolume - bucket.sellVolume; bucket.lastTrade = now; } }
