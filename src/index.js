...
async function fetchCoinLiquidations(coin, startMs, endMs) {
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    console.log(`[Pinax][REQUEST] ${coin} page=${page} limit=${PAGE_LIMIT}`);
    ...
    if (timestamps.length && Math.max(...timestamps) < startMs) break;
    if (rows.length < PAGE_LIMIT) break;
  }
  const filtered = all.filter(e => { const ts = eventTimestampMs(e); return Number.isFinite(ts) && ts >= startMs && ts < endMs; });
  console.log(`[Pinax][LOCAL_FILTER] ${coin} pages=${Math.min(MAX_PAGES, Math.ceil(all.length / PAGE_LIMIT))} fetched=${all.length} matched=${filtered.length} window=${new Date(startMs).toISOString()}..${new Date(endMs).toISOString()}`);
  return filtered;
}
...
async function fetchLiquidationSpike(startMs, endMs) { console.log(`[SPIKE][WINDOW] ${new Date(startMs).toISOString()}..${new Date(endMs).toISOString()}`); const results = await Promise.all(ASSETS.map(async coin => { try { const events = await fetchCoinLiquidations(coin, startMs, endMs); const users = new Set(); let longUsers = 0; let shortUsers = 0; let totalNotional = 0; const seenDirectionalUsers = new Set(); let sampleLogged = false; for (const e of events) { const user = getLiquidatedUser(e); if (!user) { if (!sampleLogged) { console.log(`[Pinax][USER_MISSING] ${coin} sample=${JSON.stringify(e).slice(0, 1000)}`); sampleLogged = true; } continue; } const notional = Number(e?.notional || 0); users.add(user); totalNotional += Number.isFinite(notional) ? notional : 0; const direction = String(e?.direction || e?.liquidation_kind || '').toUpperCase(); const key = `${user}:${direction}`; if (!seenDirectionalUsers.has(key)) { seenDirectionalUsers.add(key); if (direction.includes('LONG')) longUsers += 1; if (direction.includes('SHORT')) shortUsers += 1; } } const result = { coin, users: users.size, longUsers, shortUsers, totalNotional, events: events.length }; console.log(`[Pinax][STATS] ${coin} events=${events.length} uniqueLiquidated=${users.size} longUsers=${longUsers} shortUsers=${shortUsers} notional=${totalNotional}`); return result; } catch (error) { console.error(`[Pinax][ERROR] ${coin}:`, error?.message ?? error); return { coin, users: 0, longUsers: 0, shortUsers: 0, totalNotional: 0, events: 0, error: error?.message ?? String(error) }; } })); console.log(`[SPIKE][STATS] ${JSON.stringify(results)}`); return results; }
...
async function sendSpikeAlert(periodStart, stats, minutes) { const candidates = stats.filter(x => x.users > 0).sort((a, b) => b.users - a.users || b.totalNotional - a.totalNotional); console.log(`[ALERT][CANDIDATES] ${minutes}M ${JSON.stringify(candidates)}`); if (!candidates.length) { console.log(`[ALERT][NO_WINNER] ${minutes}M: no coin has liquidated users > 0`); return null; } const winner = candidates.find(x => x.coin !== lastAlertAsset); if (!winner) { console.log(`[ALERT][SKIP_REPEAT] ${minutes}M: winner=${candidates[0].coin}, lastAlertAsset=${lastAlertAsset}`); return null; } const marketStart = periodStart + minutes * 60 * 1000; const lines = ['🚨 LIQUIDATION SPIKE', '', `${winner.coin} — ${minutes}M`]; if (winner.longUsers > 0) lines.push(`LONG users: ${winner.longUsers}`); if (winner.shortUsers > 0) lines.push(`SHORT users: ${winner.shortUsers}`); lines.push(`Liquidation volume: ${Math.round(winner.totalNotional).toLocaleString('en-US')} USDT`, '', '▶️ POLYMARKET', polymarketUrl(winner.coin, marketStart, minutes)); const text = lines.join('\n'); console.log(`[ALERT][WINNER] ${minutes}M ${winner.coin} users=${winner.users} volume=${winner.totalNotional}`); await sendTelegram(text); lastAlertAsset = winner.coin; console.log(`[ALERT][SENT] ${minutes}M ${winner.coin}`); return { coin: winner.coin, users: winner.users, longUsers: winner.longUsers, shortUsers: winner.shortUsers, totalNotional: winner.totalNotional, timeframe: minutes, market: polymarketUrl(winner.coin, marketStart, minutes) }; }
...
async function start() { console.log('=== POLYMARKET LIQUIDATION SPIKE MONITOR ==='); console.log('SOURCE: PINAX HYPERLIQUID MARKET LIQUIDATIONS'); console.log('ASSETS: BTC, ETH, XRP, SOL, DOGE, HYPE, BNB'); console.log('PERIODS: 5M + 15M'); console.log('NO LIQUIDATION SIZE THRESHOLD — ALL LIQUIDATIONS COUNT'); console.log('RULE: LARGEST NUMBER OF UNIQUE LIQUIDATED USERS PER COIN'); console.log('REPEAT RULE: CROSS-TIMEFRAME — SAME COIN CANNOT ALERT TWICE IN A ROW'); console.log('TIME FILTER: local timestamp filter over paginated Pinax events'); console.log('15M WINDOW: PREVIOUS FULLY CLOSED POLYMARKET QUARTER-HOUR INTERVAL'); console.log(`RUN_ONCE: ${RUN_ONCE}`); console.log('DIAGNOSTICS: Pinax requests, response keys, local filtering, stats, winner, Telegram result'); console.log('OLD RULE REMOVED: NO MORE ALERTS FOR SINGLE LARGEST LIQUIDATION'); console.log('DIRECTION ALERT: show LONG users only when >0; show SHORT users only when >0'); await processDueWindows(); if (RUN_ONCE) { console.log('[RUN_ONCE] Diagnostic cycle complete; exiting.'); stopping = true; boundaryTimer = null; server.close(() => console.log('[RUN_ONCE] HTTP diagnostics server closed.')); return; } scheduleNextBoundary(); }
