...
const ALERT_MIN_USD = Number(process.env.CVD_ALERT_MIN_USD || 100000);
...
function newState() {
  return {
    '5M': newBucket(Math.floor(Date.now() / WINDOWS['5M']) * WINDOWS['5M']),
    '15M': newBucket(Math.floor(Date.now() / WINDOWS['15M']) * WINDOWS['15M'])
  };
}
function getState(asset) {
  let state = stats.get(asset);
  if (!state) { state = newState(); stats.set(asset, state); }
  return state;
}
function rollBuckets(state, now) {
  for (const period of Object.keys(WINDOWS)) {
    const start = Math.floor(now / WINDOWS[period]) * WINDOWS[period];
    if (state[period].start !== start) state[period] = newBucket(start);
  }
}
function addCvd(asset, isBuyerAggressor, usd, now) {
  const state = getState(asset);
  rollBuckets(state, now);
  for (const period of Object.keys(WINDOWS)) {
    ...
    const positiveCandidate = bucket.maxPositive >= ALERT_MIN_USD && bucket.cvd >= bucket.maxPositive;
    const negativeCandidate = Math.abs(bucket.maxNegative) >= ALERT_MIN_USD && bucket.cvd <= bucket.maxNegative;
    if (Date.now() - bucket.lastAlertAt >= ALERT_COOLDOWN_MS) {
      if (positiveCandidate) {
        bucket.lastAlertAt = Date.now();
        void alertCvdFlow(asset, period, bucket, 'POSITIVE', now);
      } else if (negativeCandidate) {
        bucket.lastAlertAt = Date.now();
        void alertCvdFlow(asset, period, bucket, 'NEGATIVE', now);
      }
    }
  }
}
...
async function alertCvdFlow(asset, period, bucket, direction, now) {
  ...
  const lines = [
    ...
    '', `${asset} — ${period} CVD`,
    `📊 Current CVD: ${sign}${bucket.cvd.toFixed(0)} USDT`,
    `🔥 ${direction === 'POSITIVE' ? 'Positive inflow peak' : 'Negative outflow peak'}: ${value.toFixed(0)} USDT`,
    '', '▶️ POLYMARKET',
...
function snapshot(asset) {
  ...
  for (const period of Object.keys(WINDOWS)) {
    const bucket = state[period];
    out[period] = {
      start: bucket.start, cvd: bucket.cvd,
      maxPositive: bucket.maxPositive, maxNegative: bucket.maxNegative,
      alertThresholdUsd: ALERT_MIN_USD, lastTrade: bucket.lastTrade,
...
const server = createServer((req, res) => {
  ...
  if (url.pathname === '/cvd' || url.pathname === '/trades') {
    res.writeHead(200); res.end(JSON.stringify({ ok: true, source: 'Hyperliquid realtime trades', checkedAt: new Date().toISOString(), streamConnected, lastMessageAt, assets: ASSETS, periods: ['5M', '15M'], monitor: 'CVD_FLOW_5M_15M', alerts: true, alertThresholdUsd: ALERT_MIN_USD, data: allStats() })); return;
  }
  if (url.pathname === '/health') {
    res.writeHead(200); res.end(JSON.stringify({ ok: true, service: 'polymarket-cvd-monitor', assets: ASSETS, periods: ['5M', '15M'], streamConnected, lastMessageAt, alertThresholdUsd: ALERT_MIN_USD, alerts: true })); return;
  }
...
console.log(`ALERT: STRONGEST POSITIVE INFLOW OR NEGATIVE OUTFLOW, MIN ${ALERT_MIN_USD} USDT, COOLDOWN ${ALERT_COOLDOWN_MS}ms`);
