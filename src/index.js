...
function instrumentNameOf(event, id) {
  const p = event?.payload ?? event?.data ?? event;
  return text(p?.symbol ?? p?.ticker ?? p?.instrument ?? p?.instrumentName ?? p?.instrument_name ?? p?.market ?? id);
}
...
function recordTrade(id, event) {
  const trade = tradeValues(event);
  if (!trade.notional) return;
...
function volumeStats(s, now) {
  ...
  const previous = [...s.buckets.entries()]
    .filter(([bucket]) => bucket < currentBucket)
    .sort((a, b) => b[0] - a[0])
    .slice(0, HISTORY_BUCKETS - 1)
    .map(([, value]) => value)
    .filter(v => v > 0);
...
function canAlert(key) {
  ...
  cooldowns.set(key, now + SIGNAL_COOLDOWN_MS);
  return true;
}
...
async function evaluateSignal(id, event) {
  const t = addTickerPoint(id, event);
  if (!t) return;
  const s = stateFor(id, t.symbol || instrumentNameOf(event, id));
  const now = Date.now();
  ...
  const volumeMultiple = volume.multiplier;

  const longChecks = [
    priceChange >= PRICE_MOVE_5M,
    oiChange !== null && oiChange >= OI_MOVE_5M,
    funding !== null && funding >= FUNDING_LONG,
    volumeMultiple !== null && volumeMultiple >= MIN_VOLUME_MULTIPLIER
  ];
  const shortChecks = [
    priceChange <= -PRICE_MOVE_5M,
    oiChange !== null && oiChange >= OI_MOVE_5M,
    funding !== null && funding <= FUNDING_SHORT,
    volumeMultiple !== null && volumeMultiple >= MIN_VOLUME_MULTIPLIER
  ];
