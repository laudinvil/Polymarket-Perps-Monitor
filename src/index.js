...
async function fetchHistoricalEvents(symbol, timeframe) {
  ...
  try {
    const json = await new Promise((resolve, reject) => { const parsed = new URL(url); const req = https.request({ hostname: parsed.hostname, path: `${parsed.pathname}${parsed.search}`, method: 'GET', timeout: REQUEST_TIMEOUT_MS, headers: { 'User-Agent': 'Polymarket-Perps-Monitor', 'Accept': 'application/json' } }, res => { let data = ''; res.setEncoding('utf8'); res.on('data', chunk => { data += chunk; }); res.on('end', () => { if (res.statusCode < 200 || res.statusCode >= 300) return reject(new Error(`MarginPad historical request failed: ${res.statusCode} ${data}`)); try { resolve(JSON.parse(data)); } catch (error) { reject(error); } }); }); req.on('timeout', () => req.destroy(new Error('MarginPad historical request timed out'))); req.on('error', reject); req.end(); });
