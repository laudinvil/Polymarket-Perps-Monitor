...
async function process5m(startMs) { if (lastProcessed5m === startMs) return; const endMs = startMs + WINDOW_5M; const stats = await fetchLiquidationSpike(startMs, endMs); const key = `5m:${startMs}`; let alert5m = null; if (!alertedPeriods.has(key)) { alert5m = await sendSpikeAlert(startMs, stats, 5); if (alert5m) alertedPeriods.add(key); } lastProcessed5m = startMs; return { periodStart: startMs, periodEnd: endMs, stats, alert5m }; }
async function process15m(startMs) { if (lastProcessed15m === startMs) return; const endMs = startMs + WINDOW_15M; const stats = await fetchLiquidationSpike(startMs, endMs); const key = `15m:${startMs}`; let alert15m = null; if (!alertedPeriods.has(key)) { alert15m = await sendSpikeAlert(startMs, stats, 15); if (alert15m) alertedPeriods.add(key); } lastProcessed15m = startMs; return { periodStart: startMs, periodEnd: endMs, stats, alert15m }; }
...
async function start() {
  ...
  console.log('SOURCES: PINAX HYPERLIQUID + BINANCE FUTURES FORCE ORDER');
  console.log('ASSETS: BTC, ETH, XRP, SOL, DOGE, HYPE, BNB');
  console.log('PERIODS: 5M + 15M');
  console.log('PINAX: unique liquidated users + volume');
  console.log('BINANCE: liquidation events + volume; no public user identities');
  console.log(`PINAX PAGINATION: limit=${PAGE_LIMIT}, maxPages=${MAX_PAGES_PER_COIN}, pageDelay=${PAGE_DELAY_MS}ms, coinDelay=${COIN_DELAY_MS}ms`);
  console.log('BINANCE: persistent realtime WebSocket stream; events are retained in memory for 5M/15M windows');
  console.log(`RUN_ONCE: ${RUN_ONCE}`);
  startBinanceLiquidationStream();
  await processDueWindows();
  if (RUN_ONCE) { stopping = true; stopBinanceLiquidationStream(); boundaryTimer = null; server.close(() => console.log('[RUN_ONCE] done')); return; }
