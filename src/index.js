...
const MIN_SHORT_15M = 50000;
const MIN_LONG_15M = 50000;
const MIN_SHORT_5M = 50000;
...
async function sendAlert(period, side, start, end, item) {
  if (!item) return;
  await sendTelegram([`🚨 LARGEST ${side} LIQUIDATION — ${period}`,'',`${item.asset} — ${side} LIQUIDATION`,`💥 Size: ${money(item.notional,item.quote)}`,`Price: ${item.price}`,`Qty: ${item.quantity}`,`Window: ${windowText(start,end)}`,'',`▶️ NEXT ${item.asset} ${period} UP/DOWN`,nextMarketLink(item.asset,end,period==='15M'?WINDOW_15M:WINDOW_5M)].join('\n'));
}
async function advanceWindows(now) {
  ...
  while (windowStart5m < target5) { const s=windowStart5m,e=s+WINDOW_5M; const l=largestLong5m,sh=largestShort5m; largestLong5m=null; largestShort5m=null; windowStart5m=e; await sendAlert('5M','LONG',s,e,l); await sendAlert('5M','SHORT',s,e,sh); }
  while (windowStart15m < target15) { const s=windowStart15m,e=s+WINDOW_15M; const l=largestLong15m,sh=largestShort15m; largestLong15m=null; largestShort15m=null; windowStart15m=e; await sendAlert('15M','LONG',s,e,l); await sendAlert('15M','SHORT',s,e,sh); }
}
...
async function handleForceOrder(payload) {
  ...
  const parsed=parseSymbol(order.s,side==='SELL'?ASSETS_5M:ASSETS_15M);if(!parsed)return;
  ...
  if(side==='BUY'&&notional>=MIN_SHORT_5M){const w=Math.floor(time/WINDOW_5M)*WINDOW_5M;if(w===windowStart5m){const c={asset:parsed.asset,quote:parsed.quote,price,quantity,notional};if(!largestShort5m||notional>largestShort5m.notional)largestShort5m=c;}}
  if(side==='SELL'&&notional>=MIN_LONG_15M){const w=Math.floor(time/WINDOW_15M)*WINDOW_15M;if(w===windowStart15m){const c={asset:parsed.asset,quote:parsed.quote,price,quantity,notional};if(!largestLong15m||notional>largestLong15m.notional)largestLong15m=c;}}
...
console.log('=== POLYMARKET LIQUIDATION MONITOR ===');console.log('5M: LONG >= 50000; SHORT >= 50000 USDT/USDC');console.log('15M: LONG >= 50000; SHORT >= 50000 USDT/USDC');console.log('Assets: BTC, ETH, XRP, SOL, DOGE, HYPE, BNB');console.log('Each alert has one next UP/DOWN market link');
