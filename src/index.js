...
async function start() { console.log('=== POLYMARKET LIQUIDATION SPIKE MONITOR ==='); console.log('SOURCE: PINAX HYPERLIQUID MARKET LIQUIDATIONS'); console.log('ASSETS: BTC, ETH, XRP, SOL, DOGE, HYPE, BNB'); console.log('PERIODS: 5M + 15M'); console.log('NO LIQUIDATION SIZE THRESHOLD — ALL LIQUIDATIONS COUNT'); console.log('RULE: LARGEST NUMBER OF UNIQUE LIQUIDATED USERS PER COIN'); console.log('REPEAT RULE: CROSS-TIMEFRAME — SAME COIN CANNOT ALERT TWICE IN A ROW'); console.log('TIME FILTER: local timestamp filter over paginated Pinax events'); console.log(`PAGINATION: max ${MAX_PAGES_PER_COIN} pages/coin, ${PAGE_DELAY_MS}ms page delay, ${COIN_DELAY_MS}ms coin delay, sequential coins`); console.log('15M WINDOW: PREVIOUS FULLY CLOSED POLYMARKET QUARTER-HOUR INTERVAL'); console.log(`RUN_ONCE: ${RUN_ONCE}`); console.log('DIAGNOSTICS: Pinax requests, page ranges, local filtering, stats, winner, Telegram result'); console.log('OLD RULE REMOVED: NO MORE ALERTS FOR SINGLE LARGEST LIQUIDATION'); console.log('DIRECTION ALERT: show LONG users only when >0; show SHORT users only when >0'); await processDueWindows(); if (RUN_ONCE) { console.log('[RUN_ONCE] Diagnostic cycle complete; exiting.'); stopping = true; boundaryTimer = null; server.close(() => console.log('[RUN_ONCE] HTTP diagnostics server closed.')); return; } scheduleNextBoundary(); }
