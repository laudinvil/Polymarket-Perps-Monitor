...
const PRICE_MOVE_5M = Number(process.env.PRICE_MOVE_5M ?? 0.005);
const OI_MOVE_5M = Number(process.env.OI_MOVE_5M ?? 0.05);
const FUNDING_LONG = Number(process.env.FUNDING_LONG ?? 0.0003);
const FUNDING_SHORT = Number(process.env.FUNDING_SHORT ?? -0.0003);
const MIN_VOLUME_MULTIPLIER = Number(process.env.MIN_VOLUME_MULTIPLIER ?? 3);
const VOLUME_BUCKET_MS = 5 * 60 * 1000;
const HISTORY_BUCKETS = Number(process.env.HISTORY_BUCKETS ?? 12);
const MIN_BASELINE_BUCKETS = Number(process.env.MIN_BASELINE_BUCKETS ?? 3);
const SIGNAL_SCORE = Number(process.env.SIGNAL_SCORE ?? 3);
...
function tickerValues(event) { const p = event?.payload ?? event?.data ?? event; return { symbol:text(p?.symbol ?? p?.ticker ?? p?.instrument ?? p?.market ?? ''), price:firstNum(p?.markPrice,p?.mark_price,p?.midPrice,p?.mid_price,p?.lastPrice,p?.last_price,p?.price,p?.mark,p?.last), openInterest:firstNum(p?.openInterest,p?.open_interest,p?.oi), funding:firstNum(p?.fundingRate,p?.funding_rate,p?.funding,p?.currentFundingRate), timestamp:normalizeTimestamp(firstNum(p?.timestamp,p?.ts,p?.time,Date.now())) }; }
...
async function evaluateSignal(id,event) {
  ...
  const priceChange=(t.price-old.price)/old.price; const oiChange=t.openInterest!==null&&old.oi!==null&&old.oi>0?(t.openInterest-old.oi)/old.oi:null; const funding=t.funding; const volumeMultiple=volume.multiplier;
  const longChecks=[priceChange>=PRICE_MOVE_5M,oiChange!==null&&oiChange>=OI_MOVE_5M,funding!==null&&funding>=FUNDING_LONG,volumeMultiple!==null&&volumeMultiple>=MIN_VOLUME_MULTIPLIER];
  const shortChecks=[priceChange<=-PRICE_MOVE_5M,oiChange!==null&&oiChange>=OI_MOVE_5M,funding!==null&&funding<=FUNDING_SHORT,volumeMultiple!==null&&volumeMultiple>=MIN_VOLUME_MULTIPLIER];
  ...
  await telegram([`${arrow} COMPOSITE PERPS SIGNAL`,'',s.name||String(id),'',`Perps price move: ${pct(priceChange)} / 5m`,`Perps volume: ${volumeMultiple?.toFixed(1)}× baseline`,`Perps OI: ${oiChange===null?'n/a':pct(oiChange)}`,`Perps funding: ${funding===null?'n/a':pct(funding)}`,'',`Signal: ${direction}`,`Score: ${Math.max(longScore,shortScore)}/4`,'',`Prediction market: ${prediction?.question??'not found'}`,`${direction==='LONGS ENTERING'?'Up':'Down'} price: ${price(predictionPrice)}`,prediction?`➡️ ${prediction.url}`:''].filter(Boolean).join('\n'));
}
...
async function main() { console.log('Starting Polymarket Perps Composite Signal Monitor...'); console.log('BBO / Order Book alerts are disabled.'); console.log(`Signal: price=${pct(PRICE_MOVE_5M)}/5m, OI=${pct(OI_MOVE_5M)}/5m, funding long>=${pct(FUNDING_LONG)}, short<=${pct(FUNDING_SHORT)}, volume>=${MIN_VOLUME_MULTIPLIER}x, score>=${SIGNAL_SCORE}/4`); console.log(`Prediction market required: ${REQUIRE_PREDICTION_MARKET}`); const tickerHandle=await client.subscribe([{topic:'perps.tickers'}]); for await(const event of tickerHandle){const id=instrumentIdOf(event);if(id===null)continue;const symbol=instrumentNameOf(event,id);if(!instruments.has(id)){instruments.set(id,symbol);void subscribeTrades(id);}await evaluateSignal(id,event);} }
