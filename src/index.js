...
function addTickerPoint(id,event) { const t=tickerValues(event); if(t.price===null)return; const s=stateFor(id,t.symbol||instrumentNameOf(event,id)); s.points.push({ts:t.timestamp,price:t.price,oi:t.openInterest}); const cutoff=Date.now()-15*60*1000; s.points=s.points.filter(x=>x.ts>=cutoff); return t; }
